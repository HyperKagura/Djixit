...
class CahRoom(models.Model):
    ...
    def add_connection(self, user):
        ...
        user_in_room = CahUsersInRoom.objects.create(user=user, room=self, action_required=action_required)
        if self.game_state != ROOM_GAME_STATE_WAITING_PLAYERS:
            cards = list(CahCardGame.objects.filter(room=self, card_state=CARD_STATE_WAITING))
            random.shuffle(cards)
            for card in cards[:10]:
                CahCardGame.objects.filter(id=card.id).update(user=user_in_room, card_state=CARD_STATE_IN_GAME)
...
class CahUsersInRoom(models.Model):
    ...
    def set_host(self, action_required=False):
        CahUsersInRoom.objects.filter(room=self.room).update(is_host=False)
        self.is_host = True
        self.action_required = action_required
        self.save()

    def set_next_host(self, next_host_action_required=False):
        if self.is_host:
...
def change_user_count_pre(instance, using, **kwargs):
    CahCardGame.objects.filter(room=instance.room, user=instance, card_state=CARD_STATE_IN_GAME).update(
        card_state=CARD_STATE_WAITING, user=None)
    CahCardGame.objects.filter(room=instance.room, user=instance, card_state=CARD_STATE_VOTE).update(
        card_state=CARD_STATE_PLAYED, user=None)
    if instance.is_host:
        if instance.room.game_state == ROOM_GAME_STATE_VOTING:
